...
public class DefaultHeaderSessionManager extends DefaultSessionManager implements WebSessionManager
{
    ...
    private final static int COOKIE_EXPIRE_TIME = 86400;
    ...
    private String getSessionIdHeaderValue(ServletRequest request, ServletResponse response)
    {
        ...
        else
        {
            ...
            {
                httpResponse.setHeader(TOKEN, sessionId);
    ...
    private Serializable getReferencedSessionId(ServletRequest request, ServletResponse response)
    {
        ...
        if (id == null)
        {
            if (request instanceof  HttpServletRequest)
            {
                id = HttpUtil.getCookieValue((HttpServletRequest) request, TOKEN);
            }
        }
        else
        {
    ...
    {
        response.setHeader(TOKEN, "deleteMe");
        HttpUtil.removeCookie(response, TOKEN, "/");
    }
    ...
    private void storeSessionId(Serializable currentId, HttpServletRequest ignored, HttpServletResponse response)
    {
        ...
        else
        {
            ...
            response.setHeader(TOKEN, idString);

            HttpUtil.addCookie(response, TOKEN, idString,"/", COOKIE_EXPIRE_TIME);
    ...
    private void onInvalidation(SessionKey key)
    {
        ...
        if (WebUtils.isHttp(key))
        {
            ...
            HttpUtil.removeCookie(WebUtils.getHttpResponse(key), TOKEN, "/");
        }
        else
        {
